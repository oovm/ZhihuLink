...
Unprotect[ZhihuLink`Private`Package`$TopLevelLoad];
ZhihuLink`Private`Package`$TopLevelLoad=
...
$PackageContexts={
		"ZhihuLink`",
		"ZhihuLink`Private`Package`",
		"ZhihuLink`Private`Hidden`"
		};
$PackageDeclared=
...
PackageScopeBlock[e_,scope_String:"Hidden"]/;TrueQ[$AllowPackageRescoping]:=
	With[{newcont="ZhihuLink`Private`"<>StringTrim[scope,"`"]<>"`"},
...
PackageDecontext[
	...
	With[{
		...
		ctx="ZhihuLink`Private`"<>StringTrim[scope,"`"]<>"`"
		},
		Replace[names,
...
If[(Clear@ZhihuLink`Private`Package`$loadAbort;!#)&@ZhihuLink`Private`Package`$loadAbort,
	ZhihuLink`Private`Package`PackagePostProcessContextPathReassign[]
	]
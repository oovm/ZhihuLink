...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated ZhihuLink loader file *)


Temp`PackageScope`ZhihuLinkLoading`Private`$DependencyLoad =
	TrueQ@Temp`PackageScope`ZhihuLinkLoading`Private`$DependencyLoad;
If[Temp`PackageScope`ZhihuLinkLoading`Private`$DependencyLoad,
	Unprotect["`ZhihuLink`PackageScope`Private`Constants`$TopLevelLoad"];
	Evaluate[Symbol["`ZhihuLink`PackageScope`Private`Constants`$TopLevelLoad"]] = False,
	Unprotect["ZhihuLink`PackageScope`Private`Constants`$TopLevelLoad"];
	Evaluate[Symbol["ZhihuLink`PackageScope`Private`Constants`$TopLevelLoad"]] =
		MemberQ[$ContextPath, "Global`"]
];


If[Temp`PackageScope`ZhihuLinkLoading`Private`$DependencyLoad,
	Evaluate[Symbol["`ZhihuLink`PackageScope`Private`$RootContext"]] = $Context;
	BeginPackage["`ZhihuLink`"],
	Evaluate[Symbol["ZhihuLink`PackageScope`Private`$RootContext"]] = "";
	BeginPackage["ZhihuLink`"]
]


ClearAll[ZhihuLink];
ZhihuLink::usage = "ZhihuLink is a manager head for the ZhihuLink package";
...
ZhihuLink["LoadingParameters"] := $PackageLoadSpecs
$PackageLoadSpecs =
	Merge[
		{
			With[
...
ZhihuLink["Mode"] := $PackageLoadingMode
$PackageLoadingMode =
...
ZhihuLink["Dependencies"] := $PackageDependencies;
$PackageDependencies =
...
ZhihuLink["DependencyBase"] := $PackageDependencyBase;
...
ZhihuLink["PackageListing"] := $PackageListing;
$PackageListing = <||>;
ZhihuLink["Contexts"] := $PackageContexts;
	...
	$PackageContexts =
		Join[
			If[$PackageLoadingMode === "Dependency",
				...
				{
					"ZhihuLink`",
					"ZhihuLink`PackageScope`Private`",
					"ZhihuLink`PackageScope`Package`"
				},
...
ZhihuLink["FEScopedSymbols"] := $PackageFEHiddenSymbols;
$PackageFEHiddenSymbols = {};
ZhihuLink["PackageScopedSymbols"] := $PackageScopedSymbols;
...
ZhihuLink["FileContexts"] := $PackageFileContexts;
...
ZhihuLink["DeclaredPackages"] := $DeclaredPackages;
...
ZhihuLink["LoadedPackages"] := $LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ] :=
	With[{s = PackageFileContextPath[f]},
		StringRiffle[Append[""]@Prepend[s, ZhihuLink], "`"]
	];
...
ZhihuLink::nodep = "Couldn't load dependency `` of type ``";
ZhihuLink::nodup = "Couldn't update dependency `` of type ``";
...
PackageInstallPackageDependency[dep_String, ops : OptionsPattern[]] :=
	Block[{retcode, site, path, file, tmp},
		...
		If[retcode < 300,
			...
			DeleteFile[tmp];
			file,
			Message[ZhihuLink::nodep, dep, "Package"];
			DeleteFile[tmp];
			$Failed
		]
	];
		...
		Monitor[
			MapThread[
				Check[
					...
					Message[ZhihuLink::nodep, #2, "Paclet"];
					$Failed
				]&,
				{
					pacs,
			...
			Monitor[
				MapThread[
					Check[
						...
						Message[ZhihuLink::nodup, #2, "Paclet"];
						$Failed
					]&,
					{
						StringDelete[deps, "`"],
...
$PackageErrorMessage =
	"ZhihuLink encountered exception ``";
...
PackageCatchMessage[
	...
	Replace[
		Thread[Map[Hold[MessageName[ZhihuLink, #]]&, msg], Hold],
		{
			{} :>
				Check[expr,
...
PackageFailureException[
	...
	(
		Set[MessageName[ZhihuLink, tag], body];
		PackageFailureException[
			MessageName[ZhihuLink, tag],
			body,
			ops
...
PackageAttachMainAutocomplete[] :=
	PackageAddAutocompletions[ZhihuLink,
		Table[
			Replace[{} -> None]@
				Cases[
					DownValues[ZhihuLink],
					Nest[
						Insert[#, _, {1, 1, 1}]&,
						(HoldPattern[Verbatim[HoldPattern]][
							ZhihuLink[s_String, ___]
						] :> _),
						n - 1
					] :> s,
					Infinity
...
PackagePrepPackageSymbol[] :=
	Switch[$AllowPackageSymbolDefinitions,
		None,
		If[Length@OwnValues[ZhihuLink] == 0,
			Remove[ZhihuLink],
			DownValues[ZhihuLink] = {}
		],
		False,
...
If[Temp`PackageScope`ZhihuLinkLoading`Private`$DependencyLoad,
	If[
		(Clear@`ZhihuLink`PackageScope`Private`$loadAbort;!#)&@
			`ZhihuLink`PackageScope`Private`$loadAbort,
		Unprotect[`ZhihuLink`PackageScope`Private`$LoadCompleted];
		`ZhihuLink`PackageScope`Private`$LoadCompleted = True;
		`ZhihuLink`PackageScope`Private`PackagePostProcessContextPathReassign[]
	],
	If[
		(Clear@ZhihuLink`PackageScope`Private`$loadAbort;!#)&@
			ZhihuLink`PackageScope`Private`$loadAbort,
...
Remove["Temp`PackageScope`ZhihuLinkLoading`Private`*"]